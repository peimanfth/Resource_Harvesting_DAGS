...
def handle_single_image(args):
    ...
    if image_bytes:
        ...
        return {"prediction": result, "index": index, "image": file_name}
    else:
        return {"error": "Image not found or failed to download", "index": 0}

def handler(event, context=None):
    pool_size = len(event['images'][event['index']])  # Adjust this to the number of available cores minus one, if necessary
    with Pool(pool_size) as pool:
        tasks = [(event, index, context) for index in range(len(event['images'][event['index']]))]
