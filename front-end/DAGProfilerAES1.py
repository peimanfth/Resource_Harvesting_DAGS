...
from params import MODELS_DIR_AES1 as MODELS_DIR
...
def inputPrediction(df, models):
    ...
    results = {}
    error_directory = "./logs/aes1/errors"
    ...
    os.makedirs('./logs/aes1/times', exist_ok=True)
    time_df.to_csv('./logs/aes1/times/model_time_statistics_InputSize.csv', index=False)
...
def utilPrediction(df, models):
    ...
    time_df = pd.DataFrame(time_results)
    os.makedirs('./logs/aes1/times', exist_ok=True)
    time_df.to_csv('./logs/aes1/times/model_time_statistics_Utilization.csv', index=False)
   

if __name__ == '__main__':
    # The profiling data. We should generate distinct profiling data for each DAG
    df = pd.read_csv('/home/peiman/openwhisk/logs/2024-10-22_08-40-47/test.csv')
