...
//   activeActivationsByInvoker()
  ...
  def updateLoadList(numInvokers: Int): Unit = {
    ...
    for (i <- 0 until numInvokers) {
        activeActivationsByInvoker(i.toString()) onComplete { 
 ...
 // Greedy invoker selection based on cause
  ...
  val chosen = if (invokersToUse.nonEmpty) {
    ...
    case _ =>
      val invoker = GreedyJSQBalancer.schedule(
...
object GreedyJSQBalancer extends LoadBalancerProvider {
