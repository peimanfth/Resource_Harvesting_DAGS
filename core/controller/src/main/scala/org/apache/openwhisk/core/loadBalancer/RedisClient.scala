...
class RedisClient(
  host: String = Params.redisHost,
  port: Int = Params.redisPort,
  password: Option[String] = Params.redisPassword,
  database: Int = Params.redisDatabase
) {
  ...
  def init: Unit = {
    ...
    pool = new JedisPool(poolConfig, host, port, timeout, password.orNull, database)
